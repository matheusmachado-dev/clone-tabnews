...
describe("Use case: Registration Flow (all successful)", () => {
    ...
    const createUserResponse = await fetch(
      "http:/localhost:3000/api/v1/users",
      {
        ...
        body: JSON.stringify({
          ...
          email: "registration.flow@matheusmachado.blog.br",
  ...
  test("Receive activation email", async () => {
    ...
    expect(lastEmail.sender).toBe("<contato@matheusmachado.blog.br>");
    expect(lastEmail.recipients[0]).toBe(
      "<registration.flow@matheusmachado.blog.br>",
    );
    expect(lastEmail.subject).toBe("Ative seu cadastro no blog!");
